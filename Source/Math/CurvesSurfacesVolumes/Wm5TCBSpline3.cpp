...
TCBSpline3<Real>::TCBSpline3 (int numSegments, Real* times,
    Vector3<Real>* points, Real* tension, Real* continuity, Real* bias)
    :
...
//    //assertion(mNumSegments >= 3, "Not enough segments\n");
    ...
    mA = new1<Vector3<Real> >(mNumSegments);
    mB = new1<Vector3<Real> >(mNumSegments);
    mC = new1<Vector3<Real> >(mNumSegments);
...
//----------------------------------------------------------------------------
template <typename Real>
void TCBSpline3<Real>::InitSerialized()
{
    mPoints = new1<Vector3<Real> >(mNumSegments+1);
    mTension = new1<Real>(mNumSegments+1);
    mContinuity = new1<Real>(mNumSegments+1);
    mBias = new1<Real>(mNumSegments+1);
...
//----------------------------------------------------------------------------
template <typename Real>
TCBSpline3<Real>::TCBSpline3 (int numSegments, Real* times, Vector3<Real>* points)
:
MultipleCurve3<Real>(numSegments, times)
{
    // TO DO.  Add 'boundary type' just as in natural splines.
//    //assertion(mNumSegments >= 3, "Not enough segments\n");
    ...
    // All four of these arrays have mNumSegments+1 elements.
    mPoints = points;
    mTension = new1<Real>(numSegments + 1);
    mContinuity = new1<Real>(numSegments + 1);
    mBias = new1<Real>(numSegments + 1);
...
//----------------------------------------------------------------------------
template <typename Real>
void TCBSpline3<Real>::SetPoint(int i, const Vector3<Real>& ctrl)
{
    assert(i >= 0 && i <= mNumSegments);
...
//----------------------------------------------------------------------------
template <typename Real>
Vector3<Real> TCBSpline3<Real>::GetPoint (int i) const
{
    assert(i >= 0 && i <= mNumSegments);
    return mPoints[i];
}
//----------------------------------------------------------------------------
template <typename Real>
int TCBSpline3<Real>::GetNumberOfPoints() const
{
...
{
    Vector3<Real> velocity = mB[key] + t*(mC[key]*((Real)2) +
        mD[key]*(((Real)3)*t));
