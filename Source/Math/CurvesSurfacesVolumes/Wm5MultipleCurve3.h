...
public: //Serialization
    ...
    {
        archive(cereal::virtual_base_class<Curve3<Real>>(this));

        archive(mNumSegments,
                cereal::binary_data(mTimes, sizeof(Real)*(mNumSegments+1)),
                cereal::binary_data(mLengths, sizeof(Real)*mNumSegments),
                cereal::binary_data(mAccumLengths, sizeof(Real)*mNumSegments));
    }
    ...
    void load(Archive & archive, std::uint32_t const version)
    {
        ...
        archive(mNumSegments);
        InitSerialized();
        archive(cereal::binary_data(mTimes, sizeof(Real)*(mNumSegments+1)),
                cereal::binary_data(mLengths, sizeof(Real)*mNumSegments),
                cereal::binary_data(mAccumLengths, sizeof(Real)*mNumSegments));
    }
