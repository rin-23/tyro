...
void HQuaternion::ToRotationMatrix (Matrix3f& rot) const
{
...
void HQuaternion::ToAxes(AVector* axes) const
{
    Matrix3f kRot;
    ...
    {
        axes[iCol].X() = kRot[0][iCol];
        axes[iCol].Y() = kRot[1][iCol];
        axes[iCol].Z() = kRot[2][iCol];
    }
}
//----------------------------------------------------------------------------
...
APoint HQuaternion::Rotate (const APoint& point) const
{
    HMatrix rot;
    ToRotationMatrix(rot);
    return rot*point;
...
HQuaternion& HQuaternion::Align (const AVector& v1,
                                 const AVector& v2)
{
    ...
    {
        float invLength;
        if (Math<float>::FAbs(v1[0]) >= Math<float>::FAbs(v1[1]))
        {
            // V1.x or V1.z is the largest magnitude component.
            invLength = Math<float>::InvSqrt(v1[0]*v1[0] + v1[2]*v1[2]);
        ...
        else
        {
            ...
            invLength = Math<float>::InvSqrt(v1[1]*v1[1] + v1[2]*v1[2]);
            mTuple[1] = (float)0;
