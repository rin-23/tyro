...
//----------------------------------------------------------------------------
HMatrix::HMatrix (const AVector& translate)
{
    MakeTranslation(translate);
}
//----------------------------------------------------------------------------
HMatrix::HMatrix (const AVector& v1, const AVector& v2)
{
    Quaternionf quat;
    Matrix3f mat;
    quat.Align(v1, v2);
...
//----------------------------------------------------------------------------
HMatrix& HMatrix::operator= (HMatrix&& mat)
{
    mEntry[ 0] = mat.mEntry[ 0];
    mEntry[ 1] = mat.mEntry[ 1];
    mEntry[ 2] = mat.mEntry[ 2];
...
//----------------------------------------------------------------------------
HMatrix& HMatrix::operator= (Matrix3f&& mat)
{
    mEntry[ 0] = mat[0][0];
    mEntry[ 1] = mat[0][1];
    mEntry[ 2] = mat[0][2];
...
//----------------------------------------------------------------------------
HMatrix HMatrix::operator+ (HMatrix&& mat) const
{
    HMatrix result;
    for (int i = 0; i < 16; ++i)
    {
...
//----------------------------------------------------------------------------
void HMatrix::MakeTranslation (const AVector& translation)
{
    MakeIdentity();
    mEntry[3] = translation.X();
    mEntry[7] = translation.Y();
    mEntry[11] = translation.Z();
}

void HMatrix::SetDiagonal(float m00, float m11, float m22)
{
    mEntry[ 0] = m00;
    mEntry[ 5] = m11;
    mEntry[10] = m22;
...
void HMatrix::SetTranslation(const AVector& translation)
{
    mEntry[3] = translation.X();
    mEntry[7] = translation.Y();
    mEntry[11] = translation.Z();
...
//----------------------------------------------------------------------------
APoint HMatrix::operator* (APoint&& p) const
{
    return APoint(
                  mEntry[ 0]*p[0] +
                  mEntry[ 1]*p[1] +
...
//----------------------------------------------------------------------------
AVector HMatrix::operator* (AVector&& p) const
{
    return AVector(
                   mEntry[ 0]*p[0] +
                   mEntry[ 1]*p[1] +
...
//ADED BY RINAT
Matrix3f HMatrix::GetNormalMatrix() const
{
     Matrix3f m (mEntry[ 0], mEntry[ 1], mEntry[ 2],
                 mEntry[ 4], mEntry[ 5], mEntry[ 6],
                 mEntry[ 8], mEntry[ 9], mEntry[10]
                 );
