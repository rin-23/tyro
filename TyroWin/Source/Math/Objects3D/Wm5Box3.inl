...
//----------------------------------------------------------------------------
template <typename Real>
Box3<Real>::Box3 (const Vector3<Real>& minBound,
                  const Vector3<Real>& maxBound)
{
    Center = (maxBound + minBound)/2;
    
    Axis[0] = Vector3<Real>(1,0,0);
    Axis[1] = Vector3<Real>(0,1,0);
    Axis[2] = Vector3<Real>(0,0,1);
    
    Extent[0] = Math<Real>::FAbs(maxBound.X() - minBound.X())/2;
    Extent[1] = Math<Real>::FAbs(maxBound.Y() - minBound.Y())/2;
    Extent[2] = Math<Real>::FAbs(maxBound.Z() - minBound.Z())/2;
}
//----------------------------------------------------------------------------
template <typename Real>
