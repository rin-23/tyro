...
//----------------------------------------------------------------------------
template <typename Real>
Box3<Real> MergeBoundingBoxes (const Box3<Real>& box0, const Box3<Real>& box1)
{
    if (box0.Extent[0] == 0 || box0.Extent[1] == 0 || box0.Extent[2] == 0) {
        Box3<Real> newBox = box1;
        return newBox;
    }
    
    if (box1.Extent[0] == 0 || box1.Extent[1] == 0 || box1.Extent[2] == 0) {
        Box3<Real> newBox = box0;
        return newBox;
    }
    
    return MergeBoxes(box0, box1);
}
//----------------------------------------------------------------------------
...
template WM5_MATHEMATICS_ITEM
Box3<float> MergeBoundingBoxes<float> (const Box3<float>&, const Box3<float>&);
...
template WM5_MATHEMATICS_ITEM
Box3<double> MergeBoundingBoxes<double> (const Box3<double>&, const Box3<double>&);
