...
public:
    ...
    HMatrix (const AVector& translate);
    
    // Compute a matrix that rotates vector V1 to
    ...
    // V2 = (x2,y2,z2)
    HMatrix (const AVector& v1, const AVector& v2);
    ...
    HMatrix& operator= (HMatrix&& mat);
    HMatrix& operator= (Matrix3f&& mat);
    ...
    void MakeTranslation(const AVector& translation);
    void SetDiagonal(float m00, float m11, float m22);
    void SetTranslation(const AVector& translation);
    ...
    HMatrix operator* (HMatrix&& mat) const;  // M*mat
    ...
    APoint operator* (APoint&& p) const;  // M*p
    ...
    AVector operator* (AVector&& p) const;  // M*v
    ...
    {
        archive(cereal::binary_data(&mEntry[0], sizeof(float)*16));
    }
